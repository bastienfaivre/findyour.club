...
vi.mock('@/lib/url', () => ({
  buildClubAdminUrl: vi.fn((host: string, country: string, slug: string) => `http://${host}/${country}/${slug}`),
}))
...
describe('MyClubsPage', () => {
  ...
  it('redirects to /auth/login when unauthenticated', async () => {
    ...
    await expect(MyClubsPage()).rejects.toThrow('NEXT_REDIRECT:/auth/login')
  ...
  it('redirects to /auth/totp when totpEnabled but not totpVerified', async () => {
    ...
    await expect(MyClubsPage()).rejects.toThrow('NEXT_REDIRECT:/auth/totp')
  ...
  it('renders empty-state message when user has no active memberships', async () => {
    ...
    vi.mocked(getAuthSession).mockResolvedValue({ user: { id: 'u1', totpEnabled: false, totpVerified: false } } as any)
    vi.mocked(prisma.clubMembership.findMany).mockResolvedValue([])
    const result = await MyClubsPage()
