...
export async function submitApplication(data: unknown): Promise<SubmitApplicationResult> {
  try {
    ...
    const { name, email, country, activityTypeId, location, description, schedule, contactPhone, contactAddress, howToJoin, externalWebsiteUrl, desiredSlug, turnstileToken } = parsed.data
    ...
    const activityType = await prisma.activityType.findUnique({ where: { id: activityTypeId }, select: { id: true } })
    if (!activityType) {
      return { success: false, error: 'Invalid activity type.', code: 'VALIDATION_ERROR' }
    }
